...
// povorot figuri
   ...
   glRotatef(fig_zAlpha, 0,0,1);
    ...
    // yvelichenie
//   glScalef(test/100.0, test/100.0, test/100.0);

//    peremeshenie
//   glTranslated(1, test/100.0, 1);
...
// kryg dlya lopastey
    for (double i = 0.0; i < 2*M_PI; i += M_PI/fineness){
        glBegin(GL_LINE_LOOP);
            glVertex3d(r*cos(i)/slim,
                       r*sin(i)/slim,
                       -0.25);
            glVertex3d(r*cos(i+M_PI/10)/slim,
                       r*sin(i+M_PI/fineness)/slim,
                       -0.25);
            glVertex3d(r*cos(i+M_PI/10)/slim,
                       r*sin(i+M_PI/fineness)/slim,
                       0.25);
    ...
    for (int i = 0; i < 7; i++){
        ...
        for (double a = 0.0; a < 2.0; a += 0.1){
            glBegin(GL_LINE_LOOP);
                  ...
                  glVertex3d((sin(a*0.8+1.3)*0.5-0.3-0.182)/slim,
                             (a+r)/slim,
                             (cos(a*0.8+1.3)*0.5-0.3)/slim);
                  glVertex3d((-sin(a*0.8+1.3)*0.5+0.3+0.182)/slim,
                             (a+r)/slim,
                             (-cos(a*0.8+1.3)*0.5+0.3)/slim);
                  glVertex3d((-sin((a+0.1)*0.8+1.3)*0.5+0.3+0.182)/slim,
                             ((a+0.1)+r)/slim,
    ...
    for (double i = 0.0; i < 2*M_PI; i += M_PI/fineness){
        glBegin(GL_LINE_LOOP);
            glVertex3d(r*cos(i),
                       -4,
                       r*sin(i)-0.4);
            glVertex3d(r*cos(i+M_PI/fineness),
                       -4,
                       r*sin(i+M_PI/fineness)-0.4);
            glVertex3d(r*cos(i+M_PI/fineness),
                       -4.5,
                       r*sin(i+M_PI/fineness)-0.4);
            glVertex3d(r*cos(i),
...
// paraboloid
    for (double a = 0.4; a > 0.0; a -= 0.1){
        for (double i = 0.0; i < 2*M_PI; i += M_PI/fineness){
            glBegin(GL_LINE_LOOP);
                glVertex3d(a*cos(i)/slim,
                           a*sin(i)/slim,
                           -(-a*a+2)*3+5);
                glVertex3d(a*cos(i+M_PI/fineness)/slim,
                           a*sin(i+M_PI/fineness)/slim,
                           -(-a*a+2)*3+5);
                double nextA =  a + 0.1;
                glVertex3d(nextA*cos(i+M_PI/fineness)/slim,
                           nextA*sin(i+M_PI/fineness)/slim,
                           -(-nextA*nextA+2)*3+5);
                glVertex3d(nextA*cos(i)/slim,
                           nextA*sin(i)/slim,
                           -(-nextA*nextA+2)*3+5);
            glEnd();
        }
...
//    glEnable(GL_COLOR_MATERIAL);
    float a[3] = {test/100.0f, test/100.0f, test/100.0f};
    GLfloat b[4] = {1.0, 0.0, 0.0, 0.0};
    glLightfv(GL_LIGHT0, GL_POSITION, b );
